 ...
 * 
 * So we need to find the minimum jumps
 */
 ...
 const jumpingClouds = (arr) => {
     ...
     arr.forEach((item, index) => {
         ...
         if(path.length > 2) {
             ...
             let lastButOne = path[path.length - 2];
             let lastButTwo = path[path.length - 3];
             if((lastButTwo === lastButOne - 1) && (lastButOne === lastElem - 1)) {
                 path = [...path.slice(0, path.length-2), lastElem];
             }
         }
